# MERN Ecommerce Deployment — AWS EC2
...
cd cc-practical-repos/mern-ecommerce
```
...
```bash
curl http://<EC2-PUBLIC-IP>:5000/api/products
```

Test POST:

```bash
curl -X POST http://<EC2-PUBLIC-IP>:5000/api/products \
-H "Content-Type: application/json" \
-d '{"name":"Test Product","price":100,"category":"Electronics","description":"Sample product"}'
```
...
```bash
VITE_API_URL=http://<EC2-PUBLIC-IP>:5000/api/products npm run build
```
