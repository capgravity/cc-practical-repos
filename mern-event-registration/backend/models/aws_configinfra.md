# MERN Event Registration Deployment — AWS EC2
...
cd cc-practical-repos/mern-event-registration
```
...
```bash
curl http://<EC2-PUBLIC-IP>:5000/api/events
```

Test POST:

```bash
curl -X POST http://<EC2-PUBLIC-IP>:5000/api/events \
-H "Content-Type: application/json" \
-d '{"name":"Test Event","organizer":"JM","date":"2026-05-01","location":"Lab","description":"Sample event"}'
```
...
```bash
VITE_API_URL=http://<EC2-PUBLIC-IP>:5000/api/events npm run build
```
